...
#+begin_src emacs-lisp
  (use-package halo
    ...
    (halo-center-excluded-modes nil)
    (halo-after-change-center-excluded-modes nil)
...
- =halo-center-excluded-modes=: major modes where cursor centering is disabled.
  Contrast overlays still refresh in these buffers.
- =halo-after-change-center-excluded-modes=: major modes where non-command
  buffer changes refresh overlays without recentering point.  Add modes here
  if their own output scrolling should take precedence.
