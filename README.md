...
`halo` is an experimental Emacs minor mode for quiet editing
interfaces.  It keeps the current line and nearby lines at normal contrast, then
gradually reduces the contrast of visible lines farther away from point.
...
- `halo-radius`: lines above and below point that stay at normal
  contrast.
...
- `halo-global-excluded-modes`: major modes where
  `halo-global-mode` should not enable `halo-mode`.  Derived modes are also
  excluded.
...
- `halo-global-mode` enables the buffer-local mode in ordinary
  buffers and skips minibuffers by default.
- It can update immediately after commands, so contrast follows fast cursor
  movement.  Idle refresh remains available when `halo-live-update` is
  nil.
